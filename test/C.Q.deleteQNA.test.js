...
describe('Courses->QNA->Delete QNA', () => {
    let testInstructor;
    let testCourse;
    ...
    it('Deletes a QNA in a course', async () => {        
        const response = await request(app)
            .delete(`/courses/removeQNA/${testCourse._id}`)
        .send({sectionIndex: '0', viewType: 'videos', viewIndex: 0, querryIndex: 0})

            assert.strictEqual(response.statusCode, 200);  
            assert.strictEqual(response._body, 'Deleted!');  
    });
