...
describe('Courses->QNA->Delete QNA Reply', () => {
    let testInstructor;
    let testCourse;
    ...
    it('Deletes a QNA Reply', async () => {        
        const response = await request(app)
            .post(`/courses/deleteQnaReply/${testCourse._id}`)
        .send({sectionIndex: '0', viewType: 'videos', viewIndex: 0, querryIndex: 0, replyIndex: 0})

            assert.strictEqual(response.statusCode, 200);  
            assert.strictEqual(response._body, 'Reply Deleted');  
    });
