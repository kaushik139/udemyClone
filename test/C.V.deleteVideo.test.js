...
describe('Courses->Videos->Delete Video', () => {
    let testInstructor;
    let testCourse;
    ...
    it('Deletes existing video in a course', async () => {  
        const sampleSectionIndex = 0;
        const sampleVideoIndex = 0;

        const response = await request(app)
            .delete(`/courses/deleteVideoLecture/${testCourse._id}`)
        .send({sectionIndex: sampleSectionIndex, videoIndex: sampleVideoIndex})

            assert.strictEqual(response.statusCode, 200);  
            assert.strictEqual(response._body.message, 'Video Lecture Deleted!');  
    });
